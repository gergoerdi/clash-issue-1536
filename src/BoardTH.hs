...
topEntity dips vidRead addr wr = $(memoryMap @(Either (Unsigned 8) (Unsigned 16)) [|addr|] [|wr|] $ do
    rom <- romFromFile (SNat @0x2000) [|"_build/SpaceInvaders.bin"|]
    ram <- ram0 (SNat @0x0400)
    (vid, vidAddr, vidWrite) <- conduit @(Index 7168) [|vidRead|]
    io <- port_ @(Index 7) [|peripherals dips|]
